...
namespace Site.Repositorio.Chamados
{
    public class ChamadoRepositorio : IChamadoRepositorio
    {
        ...
        public void SalvarActiveRecord(DTOChamado chamado)
        {
            if (chamado.Id > 0)
            {
                Editar(chamado);
            ...
            {
                IncluirActiveRecord(chamado);
            }
        }

        private void IncluirActiveRecord(DTOChamado chamado)
        {
            var chamadoModel = ChamadoModel.Transform(chamado);
            var sql = chamadoModel.QueryInsert();

            using (var contexto = new Contexto())
            {
                var result = contexto.ExecutaComandoComRetorno(sql);
                result.Read();
                chamado.Id = Convert.ToInt32(result["LASTID"]);
            }
        }
    }
}
